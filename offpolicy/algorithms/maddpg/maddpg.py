...
class MADDPG(Trainer):
    ...
    def shared_train_policy_on_batch(self, update_policy_id, batch):
        ...
        '''
        sym_obs=np.dot(obs.reshape(n_rollout_threads,self.num_agents,-1,2),rotation(0.5*np.pi)).reshape(n_rollout_threads,self.num_agents,-1)
        sym_share_obs=np.dot(obs.reshape(n_rollout_threads,-1,2),rotation(0.5*np.pi)).reshape(n_rollout_threads,-1)
        sym_env_acts=symmetry_action(np.array(env_acts),mapping)
        sym_next_obs=np.dot(next_obs.reshape(n_rollout_threads,self.num_agents,-1,2),rotation(0.5*np.pi)).reshape(n_rollout_threads,self.num_agents,-1)
        sym_next_share_obs=sym_next_obs.reshape(n_rollout_threads, -1)
        ...
        cent_act0=np.concatenate(cent_act, axis=-1)
        
        
        sym_cent_obs=np.dot(cent_obs.reshape(batch_size,-1,2),self.rotation(0.5*np.pi)).reshape(batch_size,-1)

        sym_cent_act = self.symmetry_action(cent_act0.reshape(-1,5), mapping).reshape(-1,15)
        sym_cent_nact = self.symmetry_action(cent_nact.reshape(-1, 5),mapping).reshape(-1,15)
        
        

        #cent_nobs shape=[1000,54]  3*18 [self_vel, self_pos, landmark_rel_positions, other_agent_rel_positions, communication] communication dim=2
        #cent_nact shape=[1000,15]  3*5  [no_action, move_left, move_right, move_down, move_up]
        ...
        #next_step_Qs is a list which dim=1 containing tensor [1000,1]
        #print('next_step_Qs',next_step_Qs[0].shape)
        
        next_step_Q = torch.cat(next_step_Qs, dim=-1)
        #print('next_step_Q', next_step_Q.shape)
        ...
        if self.use_sym_loss:
            ...
            sym_q=update_policy.critic(sym_cent_obs, sym_cent_act)
            ori_q=update_policy.critic(cent_obs, np.concatenate(cent_act, axis=-1))
            
            sym_errors = [sym_q[0] - ori_q[0]]
