...
class QMix(Trainer):
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=torch.device("cuda:0"), episode_length=None, vdn=False):
        ...
        self.use_sym_loss=self.args.use_sym_loss
        self.sym_ipt=self.args.sym_ipt
    ...
    def symmetry_action(self, origin_action, mapping):
        out = mapping[np.argmax(origin_action, axis=1)]
    ...
    def train_policy_on_batch(self, batch, update_policy_id=None):
        ...
        #print('cent_obs',cent_obs_batch.shape)
        sym_cent_obs_batch=np.dot(cent_obs_batch.reshape(26,32,-1,2),self.rotation(0.5*np.pi)).reshape(26,32,54)
        #print('sym_cent_obs_batch',sym_cent_obs_batch.shape)
        ...
        mapping = np.array([key0, key3, key4, key2, key1])
        ...
        for p_id in self.policy_ids:
            ...
            pol_all_q_seq, _ = policy.get_q_values(stacked_obs_batch, pol_prev_act_buffer_seq,policy.init_hidden(-1, total_batch_size))   
            ...
            '''                                                               
            print('stacked_obs_batch',stacked_obs_batch.shape)                                                            
            print('pol_prev_act_buffer_seq',pol_prev_act_buffer_seq.shape) 
            '''
                                                                                    
            sym_stacked_obs_batch=np.dot(stacked_obs_batch.reshape(26,96,-1,2),self.rotation(0.5*np.pi)).reshape(26,96,18)
            sym_pol_prev_act_buffer_seq= self.symmetry_action(pol_prev_act_buffer_seq.reshape(-1,5), mapping).reshape(26,96,5)                                                           
            '''
            print('sym_stacked_obs_batch',sym_stacked_obs_batch.shape)                                                            
            print('sym_pol_prev_act_buffer_seq',sym_pol_prev_act_buffer_seq.shape)    
            '''
            
            #print('stacked_act_batch',stacked_act_batch.shape)
            sym_stacked_act_batch=self.symmetry_action(stacked_act_batch.reshape(-1,5), mapping).reshape(25,96,5)     
            #print('sym_stacked_act_batch',sym_stacked_act_batch.shape)
            
            
            sym_pol_all_q_seq, _ = policy.get_q_values(sym_stacked_obs_batch, sym_pol_prev_act_buffer_seq,policy.init_hidden(-1, total_batch_size))   
            
            
                                                                               
            '''
            print('stacked_obs_batch',stacked_obs_batch[0,0])        
            print('sym_stacked_obs_batch',sym_stacked_obs_batch[0,0])     
            ...
            if policy.multidiscrete:
                ...
                sym_pol_q_seq=policy.q_values_from_actions(torch.tensor(sym_pol_all_q_seq[:-1],dtype=torch.float32), torch.tensor(sym_stacked_act_batch,dtype=torch.float32))
            #sym_agent_q_seq
            
            agent_q_out_sequence = pol_q_seq.split(split_size=batch_size, dim=-2)
            sym_agent_q_out_sequence = sym_pol_q_seq.split(split_size=batch_size, dim=-2)
            
            agent_q_seq.append(torch.cat(agent_q_out_sequence, dim=-1))
            sym_agent_q_seq.append(torch.cat(sym_agent_q_out_sequence, dim=-1))
        ...
        sym_agent_q_seq = torch.cat(sym_agent_q_seq, dim=-1)       
        ...
        #print('agent_q_seq',agent_q_seq.shape)
        ...
        sym_Q_tot_seq = self.mixer(sym_agent_q_seq, sym_cent_obs_batch[:-1]).squeeze(-1)
        '''
        print('Q_tot_seq',Q_tot_seq.shape)
        print('sym_Q_tot_seq',sym_Q_tot_seq.shape)
        
        print('Q_tot_seq',Q_tot_seq[0])
        print('sym_Q_tot_seq',sym_Q_tot_seq[0])
        '''
        ...
        if self.use_sym_loss:
        
            sym_error = (Q_tot_seq - sym_Q_tot_seq) * (1 - bad_transitions_mask)
            sym_loss = mse_loss(sym_error).sum() / (1 - bad_transitions_mask).sum()      
            
            loss=loss+self.args.sym_ipt*sym_loss
