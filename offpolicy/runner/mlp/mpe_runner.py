...
class MPERunner(MlpRunner):
    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        for step in range(self.episode_length):
            ...
            print(len(env_acts))
            print(env_acts[0].shape)
            print(rewards.shape)
            print(next_obs.shape)
            print(next_share_obs.shape)
            print(dones.shape)
            ...
            print('sym_acts:')
            print(sym_env_acts)
            print(sym_env_acts[1].shape)
            sym_next_obs=np.dot(next_obs.reshape(n_rollout_threads,self.num_agents,-1,2),rotation(0.5*np.pi)).reshape(n_rollout_threads,self.num_agents,-1)
            sym_next_share_obs=sym_next_obs.reshape(n_rollout_threads, -1)
            '''
            
                        
            sy_step_obs[p_id] = sym_obs
            sy_step_share_obs[p_id] = sym_share_obs
            sy_step_acts[p_id] = sym_env_acts
            sy_step_rewards[p_id] = rewards
            sy_step_next_obs[p_id] = sym_next_obs
            sy_step_next_share_obs[p_id] = sym_next_share_obs
            sy_step_dones[p_id] = np.zeros_like(dones)
            sy_step_dones_env[p_id] = dones_env
            sy_valid_transition[p_id] = np.ones_like(dones)
